    ...
    public function render() {
        ...
        switch ($this->type) {
            case 'text':
                ...
                break;
            case 'textarea':
                $html = '<div class="form-group row"><label  class="col-md-3 row" for="name">'.$this->title.'</label><div class="col-md-9 row"><textarea class="form-control my-editor" rows="5" id="'. $this->id .'" name="'. $this->slug .'" >'. $content .'</textarea></div><div class="col-md-3 row"><input class="form-control" type="text" readonly value="' . $this->slug . '"></div></div>';
                break; 
            case 'file':
