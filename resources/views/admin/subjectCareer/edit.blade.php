						...
						<div class="col-md-9">
							<select class="selectpicker " id="subject_id" name="subject_id" title="Choose one of the following..." data-live-search="true" tabindex="-98">
                              ...
                              @if(count($subject)!=0)
                                  ...
                                  <option @if($data['subject_id'] == $db_subject->id) selected @endif value="{{$db_subject->id}}" data-tokens="{{$db_subject->name}}" >{{$db_subject->name }} </option>
                                  @endforeach
                               @endif
                              @if(count($subject)==0)
						...
						<div class="col-md-9">
							<select class="selectpicker " id="career_id" name="career_id" title="Choose one of the following..." data-live-search="true" tabindex="-98">
                              ...
                              @if(count($career)!=0)
                                  ...
                                  <option @if($data['career_id'] == $db_career->id) selected @endif value="{{$db_career->id}}" data-tokens="{{$db_career->name}}" >{{$db_career->name }} </option>
                                  @endforeach
                               @endif
                              @if(count($career)==0)
                              <option value=""><em>(Don't have)</em></option>
