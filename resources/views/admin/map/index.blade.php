...
<div class="box">
	...
	<div class="col-md-1"><a href="{{ route('admin.map.create') }}" class="btn btn-xs btn-primary">Create</a></div>	
	</div>
	<!-- /.box-header -->
	<div class="box-body">
			...
			<tbody>
				<?php foreach ($data as $key => $value): ?>
					<tr>
						...
						<td>{{ $value->lng }}</td>
						<td>
							<a href="{{ route('admin.map.edit',['id'=>$value->id]) }}" class="btn btn-xs btn-warning">Edit</a>
...
<script type="text/javascript">
	function confirmDelete(id) {
		$.ajax({
			url : '{{ route("admin.map.getUrlDelete") }}',
			data : {id:id},
		}).done(function(data) {
			if (data == -1) {
